...
function setupRoutes(app, authManager, storage, scheduler, logger) {
  ...
  registerMaintenanceRoutes(router, { requireAuth, logger, storage });
  registerBackupRoutes(router, { requireAuth, logger, storage, respondFromCache, backupAnalyzeCache, getOnlineAgentInfo, callAgentJobBackups, sendCachedResponse, BACKUP_ANALYZE_CACHE_TTL_MS });
  registerNotificationRoutes(router, { requireAuth, logger, cleanupOauthStates, getOAuthConfig, buildOAuthRedirect, createCodeVerifier, createCodeChallenge, base64UrlEncode, createState, exchangeOAuthCode, getPublicBaseUrl, oauthStateStore });
  registerServerManagementRoutes(router, { requireAuth, logger });
  app.use(router);
}
